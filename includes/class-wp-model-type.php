<?php
/**
 * Post API: WP_Model_Type class
 *
 * @package WordPress
 * @subpackage WP_Schema
 * @since 4.6.0
 */
...
final class WP_Model_Type {
     ...
     * @since 4.6.0
     *
     * @see register_model_type()
     *
     * @param string       $model_type Post type key.
     ...
     *                                Default empty array.
     */
    public function __construct( $model_type, $args = array() ) {
        global $wpdb;
        $this->name = $model_type;
        $this->table = "{$model_type}s"; // Table name.
    ...
    public function set_props( $args ) {
         ...
         */
        $args = apply_filters( 'register_model_type_args', $args, $this->name );
         ...
         * @param string $post_type Post type key.
         */
        $args = apply_filters( "register_{$model_type}_model_type_args", $args, $this->name );
        ...
        new WP_Create_Schema($this->table, $this->schema);

        $schema_hash_key = "schema_hash_{$this->table}";
        $current_hash = get_option($schema_hash_key);
        $new_hash = md5(serialize($this->schema));

        if ($current_hash !== $new_hash) {
            // Update schema hash
            update_option($schema_hash_key, $new_hash);

            // Run the schema update
            new WP_Update_Schema($this->table, $this->schema);
        }
    }
    ...
    public function add_supports() {
        if ( ! empty( $this->supports ) ) {
            foreach ( $this->supports as $feature => $args ) {
                if ( is_array( $args ) ) {
                    add_model_type_support( $this->name, $feature, $args );
                } else {
                    add_model_type_support( $this->name, $args );
                }
            }
            unset( $this->supports );
            ...
            if (
                ...
                add_model_type_support( $this->name, 'autosave' );
            }
        } elseif ( false !== $this->supports ) {
            // Add default features.
            add_model_type_support( $this->name, array( 'title', 'editor', 'autosave' ) );
        }
    }
